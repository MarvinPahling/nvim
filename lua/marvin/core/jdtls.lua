...
local function setup_jdtls()
  ...
  local root_dir = jdtls.setup.find_root({ '.git', 'mvnw', 'gradlew', 'pom.xml', 'build.gradle', 'AndroidManifest.xml' });
  ...
  local android_sdk_path = os.getenv("ANDROID_HOME")
  if android_sdk_path then
    settings.java.androidSupport = true
    settings.java.android = {
      sdk = {
